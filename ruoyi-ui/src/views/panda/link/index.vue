...
<script>
import { listLink, getLink, delLink, addLink, updateLink } from "@/api/panda/link";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除网页链接编号为"' + siteIds + '"的数据项？').then(function() {
        return delLink(siteIds);
      }).then(() => {
        this.getList();
